...
from googlecloud_chirp.cli import app
import pytest
...
def test_config_show_defaults(mocker, tmp_path):
    # Mock config path
    mocker.patch("googlecloud_chirp.config.Path.home", return_value=tmp_path)
...
def test_list_command_no_project_error(mocker, tmp_path):
    ...
    from googlecloud_chirp.cli import config_manager
